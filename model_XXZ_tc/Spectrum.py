...
for m in range(dim):
    ...
    amplitude = abs(np.vdot(vec_m,A.dot(vec_temp)))**2
    energy_gap = E_temp - Em

    if(abs(amplitude) > 0.01):
        Amplitudes.append(amplitude)
        Energy_gaps.append(energy_gap)

np.save('Amplitudes_1st.npy',Amplitudes)
np.save('Energy_gap_1st.npy',Energy_gaps)


idx = np.argsort(eig)[2]
E_temp = eig[idx].real
vec_temp = vec[:,idx]
for m in range(dim):
...
np.save('Amplitudes_2nd.npy',Amplitudes)
np.save('Energy_gap_2nd.npy',Energy_gaps)
